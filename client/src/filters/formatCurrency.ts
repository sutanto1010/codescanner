...
Vue.filter('formatNumber', (value)=>{
  let local= store.state.app.local
  if(!value){
    return "-";
  }
  value=Number(value)
  let temp = value.toLocaleString(local.code)
  let decimalSeparatorIndex = temp.indexOf(local.decimal_separator)
  let length = temp.length
  let formattedValue=temp
  if(decimalSeparatorIndex !=-1){
    let usedIndex = decimalSeparatorIndex + 3
    if (length > usedIndex) {
      formattedValue = temp.substring(0, usedIndex)
    }
  }
