  ...
  <div class="inline-editor" :class="classes">
    <div v-if="editingValue !== null">
      ...
      <div style="display: inline-block">
        <v-icon class="mr-1" @click="cancelEdit" title="Cancel" color="red"
          >mdi-close</v-icon
        >
        <v-icon @click="updateValue" :title="$t('Save')" color="green"
          >mdi-check</v-icon
        >
      </div>
  ...
  computed: {
    ...
    formattedValue() {
      ...
      if (this.type === "password") {
        return this.formatPassword(this.internalValue);
      }
      return this.internalValue;
    },
  },
  ...
  methods: {
    ...
    formatCurrency(value) {
      try {
        ...
        let _int = this.currencyDecimalPlaces
          ? stringified.slice(0, -1 - this.currencyDecimalPlaces)
          : stringified;
        let i = _int.length % 3;
        let head = i > 0 ? _int.slice(0, i) + (_int.length > 3 ? "," : "") : "";
        let _float = this.currencyDecimalPlaces
          ? stringified.slice(-1 - this.currencyDecimalPlaces)
          : "";
        const sign = value < 0 ? "-" : "";
        return (
          sign +
    ...
    validate() {
      ...
      if (this.required && !this.editingValue) {
        ...
        this.minLength &&
        this.editingValue &&
        this.editingValue.length < this.minLength
      ) {
        errors.push("Must be at least " + this.minLength + " characters");
      } else if (
        this.maxLength &&
        this.editingValue &&
        this.editingValue.length > this.maxLength
      ) {
