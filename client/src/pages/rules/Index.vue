      ...
      <v-card-text>
        ...
        >
          <template v-slot:[`item.row_icon`]="{}">
            <v-icon color="#E84D31">mdi-circle-small</v-icon>
          </template>
          ...
          <template v-slot:[`item.actions`]="{ item }">
            <v-btn icon :to="`/rules/${item.id}`" class="mr-1">
...
export default class Index extends Vue {
  title: string = "Rules";
  icon: string = "mdi-format-list-bulleted-square";
  ownerId: number = 0;
  $refs: any;
  item: any = null;
  ...
  Delete(item) {
    let self = this;
    this.$confirm(
      `Are you sure you want to delete this rule?<br /><strong>${item.name}</strong>`,
      "Delete Rule",
      "info",
      () => {
